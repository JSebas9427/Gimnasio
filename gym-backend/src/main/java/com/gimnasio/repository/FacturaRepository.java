...
public interface FacturaRepository extends JpaRepository<Factura, Integer> {
    ...
    // ── Facturas en rango de fechas ───────────────────────────────────────
    List<Factura> findByFechaFacturaBetween(LocalDate inicio, LocalDate fin);

    List<Factura> findByFechaFacturaBetweenAndTipo(
            LocalDate inicio, LocalDate fin, TipoFactura tipo);

    // ── Suma total de detalles en rango ───────────────────────────────────
    @Query("""
        SELECT COALESCE(SUM(d.valorPagado), 0)
        FROM DetalleFactura d
        WHERE d.factura.fechaFactura BETWEEN :inicio AND :fin
    """)
    BigDecimal sumIngresosByFecha(
    ...
    """)
    BigDecimal sumIngresosByFechaAndTipo(
            @Param("inicio") LocalDate inicio,
            @Param("fin")    LocalDate fin,
    ...
    """)
    BigDecimal sumIngresosByFechaAndMetodo(
            @Param("inicio") LocalDate inicio,
            @Param("fin")    LocalDate fin,
    ...
    Long countByFechaFacturaBetweenAndTipo(
            LocalDate inicio, LocalDate fin, TipoFactura tipo);
